...
bot.on('message', async(message) => {
  ...
  bot.nodb = (user) => console.log(`К сожелению ${user.tag} нету в базе-данных.`);

  let user = await User.findOne({ guildID: message.guild.id, userID: message.author.id });
  let guild = await Guild.findOne({ guildID: message.guild.id });
  if(!user) { User.create({ guildID: message.guild.id, userID: message.author.id }); console.log(`[✅ DataBase] Участник ${message.author.username} Успешно был(а) добавлен в базу-данных`) }
  if(!guild) { Guild.create({ guildID: message.guild.id }); console.log(`[✅ DataBase] Сервер ${message.guild.name} Успешно был добавлена в базу-данных`); }   
